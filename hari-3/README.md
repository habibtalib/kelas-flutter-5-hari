...
|---|---|---|
| `Null check operator used on a null value` bila tekan "Hantar Permohonan" | `_formKey.currentState` bernilai `null` (kunci tidak dipasang pada `Form`), atau `_academicCategory!` dipanggil sedangkan pengguna belum pilih kategori | Semak `Form(key: _formKey, ...)` benar-benar dipasang; guna `validator` untuk **halang** `_submit()` sampai ke baris `!` sebelum semua medan sah |
